...
>## 2025년 12월 18일 목요일
### git branch 생성, push 후, pull request 까지
#### git branch 생성
Branch 란?
- Branch란 나뭇가지처럼 여러 갈래로 작업 공간을 나누어 독립적으로 작업할 수 있도록 도와주는 Git의 도구입니다.
- 장점
    1. 브랜치는 독립 공간을 형성하기 때문에 원본(main)에 대해 안전합니다.
  1. 하나의 작업은 하나의 브랜치로 나누어 진행되므로 체계적인 개발이 가능합니다.
    3. 특히나 Git은 브랜치를 만드는 속도가 굉장히 빠르고, 용량도 적게 듭니다.
- 그래도 브랜치 꼭 써야하나요?
    1. 일단 main 브랜치는 상용을 의미합니다. 그래서 언제든 세상에 공개되어 있습니다.
  1. 만약 상용에 에러가 있어서 고쳐야 한다면 어떻게 해야할까요?
    고객들이 사용하고 있는데, 함부로 버전을 되돌리거나 삭제할 수 있을까요?
    3. 따라서 브랜치를 통해 별도의 작업 공간을 만들고, 그곳에서 되돌리거나 삭제를 합니다.
    4. 브랜치는 완전하게 독립이 되어있어서 어떤 작업을 해도 main에는 영향을 끼치지 못하죠.
    5. 그리고 이후에 에러를 해결했다면? 그 내용을 main에 반영할 수도 있습니다!
    6. 이러한 이유 때문에 Git에서 브랜치는 정말 강력한 기능 중의 하나라고 할 수 있습니다.  
- `git branch`
  - `git branch`: 브랜치 목록 확인
  - `git branch 박강현`: '박강현' 브랜치 생성
  - `git branch -d, -D`: 브랜치 삭제(대문자: 강제 삭제, 병합되지 않은 브랜치도 삭제 가능)
...
- Git Workflow 개념
  - (1) 작업흐름
    ...
    5. **pull request**를 통해 브랜치를 main에 반영해달라는 요청을 보냅니다.
    6. 병합이 완료되면 원격 저장소에서 병합이 완료된 브랜치는 불필요하므로 삭제합니다.
    7. main에 브랜치가 병합되면 , 각 사용자는 로컬의 브랜치로 이동합니다.
    8. 병합으로 인해 변경된 원격 저장소의 main 내용을 로컬에 받아옵니다.
    9. 병합이 완료된 main의 내용을 받았으므로, 기존 로컬 브랜치는 삭제합니다.   
--------------------------한사이클 종료-----------------------------
